 ...
 * ({@link java.awt.image.BufferedImageOp}) to filter a <code>BufferedImage</code>
...
public class BufferedImageFilter extends ImageFilter implements Cloneable {
    ...
    /**
     * Filters the information provided in the
     * {@link ImageConsumer#setColorModel(java.awt.image.ColorModel) setColorModel} method
     ...
     * requested pixels.
     * @param model the {@link java.awt.image.ColorModel} to which to set the
     ...
     * zero.
     * @see ImageConsumer#setPixels(int, int, int, int, java.awt.image.ColorModel, byte[],
                                    int, int)
     */
     ...
     * @throws IllegalArgumentException if width or height are less than
     * zero.
     * @see ImageConsumer#setPixels(int, int, int, int, java.awt.image.ColorModel, int[],
                                    int, int)
     */
    public void setPixels(int x, int y, int w, int h,
        ...
        if (this.model == model) {
            ...
            if (this.model != java.awt.image.ColorModel.getRGBdefault()) {
                convertToRGB();
            }
            int dstRem = width - w;
            int srcRem = scansize - w;
    ...
    public void imageComplete(int status) {
        ...
        case STATICIMAGEDONE:
            ...
            else if (model instanceof java.awt.image.IndexColorModel) {
                int[] bandOffsets = {0};
                if (bytePixels == null) break;
                java.awt.image.DataBufferByte db = new java.awt.image.DataBufferByte(bytePixels,
                                                       width*height);
                wr = java.awt.image.Raster.createInterleavedRaster(db, width, height, width,
                                                    1, bandOffsets, null);
            }
            else {
                convertToRGB();
            ...
            java.awt.image.BufferedImage bi = new BufferedImage(model, wr,
                                                 model.isAlphaPremultiplied(),
                                                 null);
            bi = bufferedImageOp.filter(bi, null);
            java.awt.image.WritableRaster r = bi.getRaster();
            ...
            if (cm instanceof java.awt.image.DirectColorModel) {
                java.awt.image.DataBufferInt db = (java.awt.image.DataBufferInt) r.getDataBuffer();
                consumer.setPixels(0, 0, w, h,
                                   cm, db.getData(), 0, w);
            }
            else if (cm instanceof IndexColorModel) {
                java.awt.image.DataBufferByte db = (DataBufferByte) r.getDataBuffer();
                consumer.setPixels(0, 0, w, h,
                                   cm, db.getData(), 0, w);
            }
            else {
    ...
    private final java.awt.image.WritableRaster createDCMraster() {
        WritableRaster wr;
        java.awt.image.DirectColorModel dcm = (DirectColorModel) model;
        ...
        java.awt.image.DataBufferInt db = new DataBufferInt(intPixels, width*height);
