...
public class ComponentColorModel extends java.awt.image.ColorModel {
    ...
    private static int bitsHelper(int transferType,
                                  ...
                                  boolean hasAlpha) {
        int numBits = java.awt.image.DataBuffer.getDataTypeSize(transferType);
    ...
    private void setupLUTs() {
        ...
        if (is_sRGB) {
            ...
            nonStdScale = false;
        } else if (java.awt.image.ColorModel.isLinearRGBspace(colorSpace)) {
            ...
            nonStdScale = false;
            if (transferType == java.awt.image.DataBuffer.TYPE_BYTE) {
            ...
            nonStdScale = false;
            fromsRGB8LUT16 = java.awt.image.ColorModel.getsRGB8ToLinearRGB16LUT();
            if (java.awt.image.ColorModel.isLinearGRAYspace(ics)) {
                is_LinearGray_stdScale = true;
                if (transferType == java.awt.image.DataBuffer.TYPE_BYTE) {
                    tosRGB8LUT = java.awt.image.ColorModel.getGray8TosRGB8LUT(ics);
                } else {
                    tosRGB8LUT = java.awt.image.ColorModel.getGray16TosRGB8LUT(ics);
                }
            } else {
                if (transferType == java.awt.image.DataBuffer.TYPE_BYTE) {
                    tosRGB8LUT = java.awt.image.ColorModel.getGray8TosRGB8LUT(ics);
                    fromLinearGray16ToOtherGray8LUT =
                        java.awt.image.ColorModel.getLinearGray16ToOtherGray8LUT(ics);
                } else {
                    tosRGB8LUT = java.awt.image.ColorModel.getGray16TosRGB8LUT(ics);
                    fromLinearGray16ToOtherGray16LUT =
                        java.awt.image.ColorModel.getLinearGray16ToOtherGray16LUT(ics);
                }
            }
   ...
   /** Returns an array of unnormalized color/alpha components given a pixel
     ...
     * <CODE>DataBuffer.TYPE_FLOAT</CODE>, or <CODE>DataBuffer.TYPE_DOUBLE</CODE>.
     */
    public ColorModel coerceData (java.awt.image.WritableRaster raster,
                                  boolean isAlphaPremultiplied) {
        if ((supportsAlpha == false) ||
    ...
    public java.awt.image.WritableRaster createCompatibleWritableRaster (int w, int h) {
        ...
        case java.awt.image.DataBuffer.TYPE_BYTE:
        case java.awt.image.DataBuffer.TYPE_USHORT:
            raster = java.awt.image.Raster.createInterleavedRaster(transferType,
                                                    w, h,
                                                    numComponents, null);
            break;
        default:
            java.awt.image.SampleModel sm = createCompatibleSampleModel(w, h);
            java.awt.image.DataBuffer db = sm.createDataBuffer();
     ...
     *
     */
    public java.awt.image.WritableRaster getAlphaRaster(WritableRaster raster) {
        if (hasAlpha() == false) {
            return null;
        }
