...
public class RescaleOp implements BufferedImageOp, RasterOp {
     ...
     * must be the same.
     */
    private boolean canUseLookup(java.awt.image.Raster src, java.awt.image.Raster dst) {
        ...
        if (canUseLookup(src, dst)) {
            ...
            if (dstNgray == 256) {
                ByteLookupTable lut = createByteLut(scaleFactors, offsets,
                                                    numBands, srcNgray);
                java.awt.image.LookupOp op = new java.awt.image.LookupOp(lut, hints);
                op.filter(src, dst);
            } else {
                ShortLookupTable lut = createShortLut(scaleFactors, offsets,
                                                      numBands, srcNgray);
                java.awt.image.LookupOp op = new LookupOp(lut, hints);
                op.filter(src, dst);
            }
        } else {
            //
